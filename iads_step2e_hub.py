...
from ryu.lib import hub  # 测试这个导入是否有问题
...
class IADSApp(simple_switch_13.SimpleSwitch13):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("IADS Step2E: Hub imported but not used")
    
    @set_ev_cls(ofp_event.EventOFPSwitchFeatures, CONFIG_DISPATCHER)
    def switch_features_handler(self, ev):
        super(IADSApp, self).switch_features_handler(ev)
        datapath = ev.msg.datapath
        self.logger.info("IADS: Switch {} connected - hub available but not used".format(datapath.id))
